...
- **On Cloud:** `git add`/`commit`/`push origin main` yourself, right after writing — the Stop hook here (`~/.claude/stop-hook-git-check.sh`) only checks/nags on git state, it never commits or pushes anything, so don't wait for it to do the job. Confirm the push actually succeeded before telling Pierre it's saved; if it fails, say so plainly and that the change only exists in this ephemeral sandbox until it's pushed (see §0's Cloud flow).
